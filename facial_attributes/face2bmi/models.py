...
class FacePrediction(object):
    ...
    def train(self, train_data, valid_data, bs, epochs, callbacks):
        ...
        self.model.fit(x=train_gen, steps_per_epoch=len(train_data) // bs, epochs=epochs, 
    ...
    def predict(self, img_dir, show_img = False):
        ...
        preds = self.model.predict(arr, verbose=1)
    ...
    def predict_df(self, img_dir):
        ...
        image_ids = [int(x.split("_")[0]) for x in fnames if ".jpg" in x]
        face_ids = [int(x.split("face")[1].split(".")[0]) for x in fnames if ".jpg" in x]
